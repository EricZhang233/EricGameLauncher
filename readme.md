...
无论您的游戏来自 Steam、Epic Games，还是独立的 exe，这里都是它们统一的家。
...
*   **一键扫描**：支持Steam、Epic Games、Xbox的游戏一键扫描导入，管理游戏更轻松！
*   **三段式删除防误删**：主界面删除仅移入回收站；回收站删除进入 72 小时倒计时；到期后下次启动自动彻底清理，可随时还原。
...
*   **智能扫描与一键导入**：
    *   **全自动扫描**：原生支持 Steam、Epic Games 及 Xbox (UWP) 三大平台已安装游戏的自动识别、路径提取与图标抓取。
    *   **一键全部导入**：扫描完成后支持一键将所有识别到的本地游戏安全入库，彻底告别繁琐的手动添加。
    *   **失效检测**：自动识别已卸载或路径失效的项目，支持批量清理。
    *   **Xbox/UWP 全量识别**：深度支持 Xbox/Microsoft Store 游戏包的高精度识别与图标提取。
*   **自动检查更新支持**：
    *   **内置更新检查**：启动时自动检查 GitHub Releases，确保您始终使用最新版本。
    *   **更新通道**：支持「正式版」与「开发版」双通道切换，自由选择稳定或尝鲜。
    *   **静默提示**：更新可用时，界面右上角版本号会变红且显示更新图标，我们不要扰人的弹窗提示！
    *   **一键下载**：发现新版本？一键完成下载和安装，无需手动操作。
*   **公告中心**：
    *   **云端公告**：启动时自动拉取云端公告，第一时间获取项目动态与重要通知。
    *   **已读管理**：已读状态本地持久化，跨次启动保持阅读记录。
    *   **多语言正文**：公告支持中英文双语展示。

### 专为极客设计
*   **完全便携 (Portable Mode)**：支持将所有数据（配置、缓存图标）存储在程序目录下。放入 U 盘，您的游戏库随身携带，插在任何电脑上即可运行。
*   **数据无缝迁移**：想从系统安装模式切换到便携模式？一键迁移，所有配置和图标自动搬家，无需重新设置。
*   **智能图标提取**：自动解析 `.exe`、`.lnk` 甚至 Steam/Epic Games URL 协议，提取并缓存高清图标。
...
*   **原生性能**：
  ...
  *   采用 YAML (`YamlDotNet`) 配置格式，人机皆可读；配置文件拆分为 `settings.yaml` 和 `items.yaml`，高频修改零 IO 负担。
*   **简洁架构**：
...
2.  解压并运行 `EricGameLauncher.exe`（无需安装，绿色便携）。
3.  点击右上角 **"更多" → "扫描"** 自动搜索本地游戏，在扫描结果中勾选需要导入的项目后一键入库；或通过 **"添加"** 手动选择游戏快捷方式或可执行文件。
